...
import { Image as ImageIcon, Plus } from "phosphor-react-native";
...
export default function PhotoUploadSection({
  ...
  const deleteCurrentImage = () => {
    ...
    else if (selectedImageIndex >= newPhotos.length) setSelectedImageIndex(newPhotos.length - 1);
  };
    ...
    <View style={styles.container}>
      ...
      {photos.length === 0 ? (
        ...
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <ImageIcon size={32} color={Colors.reds[500]} weight="regular" />
          <ThemedText style={styles.uploadText}>Add images</ThemedText>
          <ThemedText style={styles.uploadHint}>Pick a plan to add more media types</ThemedText>
        </TouchableOpacity>
      ) : (
        /* Filled State: 3-Column Square Grid */
        <View style={styles.grid}>
          {photos.map((uri, index) => (
            <View key={index} style={[styles.imageCard, { width: SQUARE_SIZE, height: SQUARE_SIZE }]}>
          ...
          {photos.length < 5 && (
            <TouchableOpacity 
              style={[styles.addSquare, { width: SQUARE_SIZE, height: SQUARE_SIZE, borderColor: '#E5E7EB' }]} 
              onPress={pickImage}
              activeOpacity={0.7}
            >
              <Plus size={24} color={Colors.reds[500]} weight="bold" />
            </TouchableOpacity>
          )}
        </View>
      )}
      ...
      <Modal visible={previewVisible} transparent animationType="fade">
        <SafeAreaView style={styles.modalBg}>
          ...
          <View style={styles.modalContent}>
             <Image source={{ uri: photos[selectedImageIndex] }} style={styles.previewImage} resizeMode="contain" />
          </View>
          <TouchableOpacity style={styles.modalDelete} onPress={deleteCurrentImage}>
